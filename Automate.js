    ...
    switch (fileType) {
        case "pdf":
            if (lastFile !== "" && currFile === lastFile) {
                log(chalk.magenta("Drawing Data Copied to PDF!"));
            } else {
...
const readDirectory = (path) => {
    ...
    log(chalk.cyan("Total Files = ", files.length));
    log(chalk.cyan("Total Runs Required = ", Math.ceil(files.length / 20)));
    ...
    for (let i = 0; i < (files.length < 20 ? files.length : 20); i++) {
        ...
        log("lastFile: " + lastFile);
        log("nextFile: " + nextFile);
